...
module.exports = function plugin(data) {
  ...
  return function through(files, metalsmith, done) {
    ...
    try {
      ...
      for (var name in extras.filters) {
        extras.useFilter(swig, name);
      }
    } catch(e) {
      console.log('swig-extras not found.');
    }

    /**
     * Determines if a given path is an existing file.
     *
     * @example {{filePath|isFile}}
     */
    ...
    swig.setFilter('isSlugMatch', function(input, compareTo) {
      var normalize = function(string) {
        return _str.slugify(string).toLowerCase();
      };
      return (normalize(input) == normalize(compareTo));
    });

    /**
     * Take the input and provides a truncated version…
     *
     * @example {{varName|limit}} or {{varName|limit(120)}}
     */
     ...
     * Cache buster…
     *
     * @example {{/path/to/image.jpg|bustcache}}
     */
